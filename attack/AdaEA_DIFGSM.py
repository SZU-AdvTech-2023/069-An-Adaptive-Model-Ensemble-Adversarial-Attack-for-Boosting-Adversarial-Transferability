...
class AdaEA_DIFGSM(AdaEA_Base):
    ...
    def attack(self, data, label, idx=-1):
        ...
        for i in range(self.iters):
            ...
            # rand_noise = torch.rand_like(adv_data) * 0.1 - 0.05 # cifar10
            # rand_noise = torch.rand_like(adv_data) * 0.08 - 0.04 # cifar100
            rand_noise = torch.rand_like(adv_data) * 0.06 - 0.03 # imagenet

            outputs = [self.models[idx](self.input_diversity(adv_data + rand_noise)) for idx in range(len(self.models))]
