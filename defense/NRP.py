...
https://github.com/Muzammal-Naseer/NRP#What-Can-You-Do
'''
...
class NRP_defense(nn.Module):
    def __init__(self,
                 eps: float = 16 / 255,
                 device: torch.device = torch.device('cuda')):
        super(NRP_defense, self).__init__()
        self.eps = eps
        self.device = device

        self.net_G = NRP(3, 3, 64, 23)
        self.net_G.load_state_dict(torch.load('/home/AdaEA/defense/NRP.pth'))
        self.net_G.eval()
    ...
    def forward(self, img):
        ...
        with torch.no_grad():
            purified = self.net_G(img_m).detach()
