"""
一个降噪器，在输入分类网络之前需要进行降噪
https://github.com/lfz/Guided-Denoise/blob/master/nips_deploy/defense.py
"""
...
class Net(nn.Module):
    def __init__(self, input_size, block, fwd_out, num_fwd, back_out, num_back, n, hard_mining=0, loss_norm=False):
        ...
        self.denoise = Denoise(input_size[0], input_size[1], block, 3, fwd_out, num_fwd, back_out, num_back)
        self.net = Inception3(self.denoise)

    def forward(self, adv_x):
...
class Denoise_defense(nn.Module):
    def __init__(self,
                 path: str = "/home/AdaEA/defense/denoise_incepv3_012.ckpt",
                 device: torch.device = torch.device('cuda')):
        super(Denoise_defense, self).__init__()
        self.path = path
        self.device = device
        self.incepv3model = get_model()
        self.checkpoint = torch.load(self.path)
        if isinstance(self.checkpoint, dict) and 'state_dict' in self.checkpoint:
            self.incepv3model.load_state_dict(self.checkpoint['state_dict'])
        else:
            self.incepv3model.load_state_dict(self.checkpoint)
        self.incepv3model.to(self.device)
        self.incepv3model.eval()

    def forward(self, img, gt):
        mean_tf = autograd.Variable(
            torch.from_numpy(np.array([0.5, 0.5, 0.5]).reshape([1, 3, 1, 1]).astype('float32')).cuda())
        std_tf = autograd.Variable(
            torch.from_numpy(np.array([0.5, 0.5, 0.5]).reshape([1, 3, 1, 1]).astype('float32')).cuda())
        img = (img - mean_tf) / std_tf
        with torch.no_grad():
            predict = self.incepv3model(img)[-1]
            sum = (predict.argmax(1) != (gt)).detach().sum().cpu()
            return sum
